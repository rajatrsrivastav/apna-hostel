...
async function reviewStudent(email: string) {
  return getDb().transaction(async (tx) => {
    ...
    await tx
      ...
      .values({
        userId: user.id,
        fullName: "Razorpay TEST STUDENT",
        phone: "",
        course: "ITI",
        trade: "Test",
        studyYear: "Test",
  ...
  endpoints: {
    signInReview: createAuthEndpoint(
      ...
      async (ctx) => {
        ...
        const email = process.env.RAZORPAY_REVIEW_EMAIL?.trim().toLowerCase();
        const password = process.env.RAZORPAY_REVIEW_PASSWORD;
        if (
          !email ||
